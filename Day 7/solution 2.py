...
class Directory:
	...
	def add_dir(self, name, ptr):
		...
		# catalog size of new directory
		global dir_sizes
		dir_sizes.append(new_dir_size)
...
# calculate size required to be freed based on total size usage
size_used = root.get_size()
size_free = SIZE_TOTAL - size_used
size_to_free = SIZE_REQ - size_free
print(f'Size that needs to be freed up: {size_to_free}')

# sort smallest to largest and iterate until one is large than size to free requirement
dir_sizes.sort()
for size in dir_sizes:
	if size >= size_to_free:
		print(f'Smallest directory with enough size has {size}')
		break
